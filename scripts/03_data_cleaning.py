...
def load_data(file_path, sample_size=None, chunk_size=None):
...
def clean_data(df):
    ...
    for feature in audio_features:
        if feature in df.columns:
            ...
            if missing_count > 0:
                ...
                print(f"   Filled {missing_count:,} missing values in '{feature}' with median")
    ...
    for field in text_fields:
        if field in df.columns:
            ...
            if missing_count > 0:
                ...
                print(f"   Filled {missing_count:,} missing values in '{field}' with 'Unknown'")
    ...
    if 'popularity_score' in df.columns:
        df['popularity_score'] = pd.to_numeric(df['popularity_score'], errors='coerce').fillna(0).astype(int)
    ...
    features_0_1 = ['energy', 'danceability', 'valence', 
                    'acousticness', 'instrumentalness']
...
def main():
    ...
    parser.add_argument('--input', default='data/raw/spotify_data.csv',
                       help='Input CSV file')
    parser.add_argument('--output', default='data/processed/spotify_cleaned.csv',
